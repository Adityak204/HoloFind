...
from utils.search_utils import fetch_and_parse_url
...
# Shared memory: vector DB indexing metadata (simulate)
indexed_url_cache = set()


# -- STATE SCHEMA --
class WebCrawlState(Dict):
    sub_queries: List[str]
    crawled_docs: List[
        Dict
    ]  # List of {"content": ..., "source_url": ..., "metadata": {...} }


# -- NODE 1: Expand each sub-query into URLs via Web Search --
async def search_step(state: WebCrawlState):
...
# -- NODE 2: Async Crawl + Parse + Fallback --
async def crawl_step(state):
...
def dedup_step(state):
    ...
    return {"sub_queries": state["sub_queries"], "crawled_docs": filtered_docs}


# -- NODE 4: Chunk + Embed + Store --
async def store_step(state):
    await chunk_embed_store_documents(state["crawled_docs"])
    return state  # Final state ready
